...
findFactors num x  = if ((head x)+1)^2 > num
                     then (reverse x) ++ [num `div` f | f <- x, not $ (num `div` f) `elem` x]
                     else findFactors num $ (minFactor num ((head x)+1)) : x
                           
-- Function that checks if the input number is a prime by trial division (stopping at square root)
isPrime :: Integral a => a -> Bool
isPrime 1 = False
isPrime num = checkPrime num 2
                where checkPrime num x
                        | x*x > num = True
                        | otherwise = if num `isDivisibleBy` x
                                      then False
                                      else checkPrime num (x+1)
